...
def parse_file(file_path):
    ...
    for line in lines:
        ...
        if line.startswith("'"):
            ...
            endpoints_match = re.match(r'endpoints = \[(.+)\]', line)
            if endpoints_match:
                endpoints_values = endpoints_match.group(1).split(',')
                data['endpoints'] = [eval_expression(v.strip(), data) for v in endpoints_values]
        else:
...
def main():
    if len(sys.argv) != 3:
        print("Используйте команду: python HW3/config3.py HW3/<input_file> HW3/<output_file>")
        return
